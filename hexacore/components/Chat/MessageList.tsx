...
export default ({id}) =>{
    const qMessages = query(collection(db, id+'/Messages/'), orderBy('sentAt', 'desc'), limit(25))
    ...
    const getNextMessages = async () => {
        ...
        const nextMessages = query(collection(db, id+'/Messages/'), orderBy('sentAt', 'desc'), limit(25), startAfter(lastMessage.message.sentAt))
    ...
    useEffect(()=>{
        ...
        const unsubscribe = onSnapshot(qMessages, async (querySnapshot) =>{
            ...
            setMessages(messages => messages.sort((a, b) => b.message.sentAt - a.message.sentAt))          
        })
        return () => {
            unsubscribe()
        }
